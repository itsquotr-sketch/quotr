...
export default async function ScopeDetailPage({ params }: ScopeDetailPageProps) {
  ...
  const photoUrls = await Promise.all(
    ...
    (scope.scope_photos ?? []).map(async (photo) => ({
      ...
      url: await getSignedStorageUrl("scope-photos", photo.storage_path),
  ...
  const documentUrls = await Promise.all(
    ...
    (scope.scope_documents ?? []).map(async (doc) => ({
      ...
      url: await getSignedStorageUrl("scope-documents", doc.storage_path),
  ...
  return (
    ...
    <div>
      ...
      <PageHeader title={scope.name} backHref={`/projects/${id}`} />
