-- Idempotent ensure/repair for project_scope_suggestions (Phase 1.6)
-- Safe to run if 007 was skipped or partially applied
...
alter table public.project_scope_suggestions
  drop constraint if exists project_scope_suggestions_status_check;

alter table public.project_scope_suggestions
  add constraint project_scope_suggestions_status_check check (
    status in ('pending', 'accepted', 'rejected', 'converted')
  );

alter table public.project_scope_suggestions
  drop constraint if exists project_scope_suggestions_confidence_check;

alter table public.project_scope_suggestions
  add constraint project_scope_suggestions_confidence_check check (
...
drop index if exists public.project_scope_suggestions_pending_unique_idx;
...
drop policy if exists "Users can view org scope suggestions"
  on public.project_scope_suggestions;

drop policy if exists "Users can create org scope suggestions"
  on public.project_scope_suggestions;

drop policy if exists "Users can update org scope suggestions"
  on public.project_scope_suggestions;

drop policy if exists "Users can delete org scope suggestions"
  on public.project_scope_suggestions;
